...
<table>
  <tr>
    <td>
      <table>
        <tr><td>기간</td><td>2025.07.18 ~ 2025.07.20 (3일) + 폴리싱 1개월</td></tr>
        <tr><td>인원</td><td>5명(기획 1, 클라이언트 2, 아트 2)</td></tr>
        <tr><td>역할</td><td>클라이언트 개발 리드</td></tr>
...
넥슨 '재밌넥'에서 2박 3일간 제작된 프로젝트로, 이후 폴리싱 작업을 거쳐 스토브에 출시하였습니다.
...
### 1️⃣ Audio Clock 기반 비트 동기화 시스템

#### 📌 해결하고자 한 문제
게임 로직은 프레임 기반 시간(`Time.time` / `Time.deltaTime`)을, 음악 재생은 오디오 DSP 클럭을 기준으로 진행되어 **두 시간 기준이 분리**되어 있습니다. 이 때문에 프레임 드랍이 발생하면 게임 로직만 밀리면서 음악 BPM과 노드 도착 타이밍의 오차가 누적되어, 리듬 게임의 핵심 경험이 훼손되는 문제를 해결하고자 했습니다.

#### 🔧 구현 방법
사운드 하드웨어 샘플 레이트 기반의 `AudioSettings.dspTime`을 **마스터 클럭**으로 채택하여, 음악과 동기화가 필요한 모든 로직(시간 진행/노드 이동/노드 생성)의 시간 기준을 통일했습니다. 또한 `PlayScheduled` **예약 재생**을 통해 **첫 노드가 판정 지점에 도달하는 시점과 음악 첫 박자가 일치하도록 이동 시간을 역산**하여 재생 시점을 예약했습니다.

| 비교 항목 | 기존 방식 (Frame Clock) | 개선 방식 (Audio Clock) |
|---|---|---|
| 시간 기준 (API) | `Time.time` / `Time.deltaTime` | `AudioSettings.dspTime` |
| 클럭 갱신 주체 | 메인 루프 (프레임마다 누적) | 오디오 시스템 (하드웨어 샘플 카운트) |
| 클럭 소스 | 엔진 프레임 타이밍 (프레임 의존) | 사운드 카드 (하드웨어 기반) |
| 프레임 드랍 영향 | 싱크 밀림 누적 및 박자 이탈 발생 | 부하와 무관하게 음악과 동기화 유지 |
...
#### ✨ 달성 효과
- **프레임 독립적 동기화** : 프레임 드랍 시에도 오차가 누적되지 않아, 음악 비트와 노드 판정 시점이 정확히 일치합니다.
- **런타임 변수 대응** : 일시정지 중에도 계속 흐르는 `dspTime`의 특성을 보정하여, 재개 이후에도 동일한 정밀도의 비트 플레이 환경을 유지합니다.
...
#### 📌 해결하고자 한 문제
기획자가 **위/아래/왼쪽/오른쪽 4방향**의 패턴 데이터(각 11자리 문자열)를 텍스트로만 확인·수정해야 해, **자릿수를 세어가며 작업하는 번거로움과 입력 실수**가 발생하는 문제를 해결하고자 했습니다.

#### 🔧 구현 방법
`CustomPropertyDrawer`를 활용하여 기존 문자열 입력은 유지한 채, 동일한 데이터를 **시각적인 버튼 그리드**로 표현하고 값에 따라 색상을 구분(1~4: 초록, 0: 빨강)하여 클릭만으로 편집 가능하도록 커스텀 에디터를 제작했습니다.
...
#### ✨ 달성 효과
- **생산성 향상** : 자릿수를 세지 않고도 패턴 배치를 색상으로 즉시 확인하고, 클릭만으로 수정할 수 있는 작업 환경 제공
- **리스크 최소화** : 버튼 편집 시 값의 범위(0~4)와 11자리 형식이 항상 유지되어, 오타·자릿수 실수가 발생할 수 없는 입력 경로 확보
